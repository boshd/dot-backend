...
| OpenAI | Terra: $2/M input, $0.20/M cached input, and $12/M output. Luna: $0.20/M input, $0.02/M cached input, and $1.20/M output. Embeddings: $0.02/M tokens. Web search: $0.01/call plus tokens | Dedicated project limits; low reasoning for normal work; app builds default to Terra with one bounded repair pipeline, at most five source candidates, and an early clean-regeneration/stop rule when browser acceptance stops converging. Rejected source is not regenerated through three duplicate queue attempts. Keep at most three concurrent builds and 20 active code apps per user, recording tokens/latency/repair count per build. A/B Luna on the same eval set before routing simple builds there; it is 10x cheaper at equal token counts. Codex is not assumed cheaper and Dot already owns the compiler/test/repair harness. Inbound media uses low-detail images, 20 MB/item, 45 MB/request, and eight remote items/request | [Pricing](https://developers.openai.com/api/docs/pricing), [code generation](https://developers.openai.com/api/docs/guides/code-generation), [spend limits](https://developers.openai.com/api/docs/guides/spend-limits) |
